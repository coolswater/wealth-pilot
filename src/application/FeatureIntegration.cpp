...
#include "domain/analysis/StockScreener.h"
#include "domain/backtest/BacktestEngine.h"
#include "domain/analysis/RiskAnalyzer.h"

// 中期规划功能
#include "services/social/StrategyShareManager.h"
#include "presentation/chart/DrawingToolManager.h"
#include "domain/quant/QuantTradingEngine.h"

// 长期规划功能
#include "infrastructure/ai/core/AIAssistant.h"
#include "services/account/MultiAccountManager.h"
#include "services/security/PermissionManager.h"
#include "infrastructure/api/DataAPIManager.h"
#include "infrastructure/plugins/core/PluginMarketManager.h"

// 性能管理
#include "services/performance/PerformanceManager.h"
...
void FeatureIntegration::registerDefaultShortcuts()
{
    ...
    shortcutMgr->registerShortcut(
        ...
        [this]() {
            LayoutManager::instance()->saveCurrentLayout("current");
        },
        "文件",
        tr("保存当前窗口布局")
    );
    ...
    shortcutMgr->registerShortcut(
        ...
        [this]() {
            LayoutManager::instance()->loadLayout("current");
        },
        "文件",
        tr("恢复默认窗口布局")
    );
