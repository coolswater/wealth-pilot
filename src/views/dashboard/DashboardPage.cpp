    ...
    , d(std::make_unique<Impl>())
{
    ...
    connect(d->m_realtimeTimer, &QTimer::timeout, this, &DashboardPage::updateRealTimeData);
}
...
void DashboardPage::createHeader()
{
    ...
    connect(d->m_searchEdit, &QLineEdit::textChanged, this, &DashboardPage::onSearch);
    ...
    connect(timeRange, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &DashboardPage::onTimeRangeChanged);
    layout->addWidget(timeRange);

    d->mainLayout->addWidget(header);   // 添加整个头部
}

void DashboardPage::createCardsSection()
{
    // 总资产卡片
...
void DashboardPage::createChartsSection()
{
    // 资产配置饼图
...
void DashboardPage::setupDemoData() const
{
    // 更新卡片数据（带动画）
    d->m_totalAssetCard->setValue("¥1,245,680", true);
    d->m_dailyPnLCard->setValue("+¥28,450", true);
...
void DashboardPage::onSearch(const QString& text)
{
...
void DashboardPage::onTimeRangeChanged(int index) const
{
    // 根据时间范围重新加载图表数据
...
void DashboardPage::updateRealTimeData() const
{
    // 模拟实时价格更新
