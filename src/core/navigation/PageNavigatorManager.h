 ...
 * - WeakCache: 默认策略，平衡内存占用和响应速度，允许系统在内存紧张时自动回�?
 * - StrongCache: 适用于高频核心页面（如首页Dashboard），常驻内存确保零延迟切�?
 */
...
/**
 * @brief 导航历史记录条目
 * @details 保存页面跳转历史，支持前进后退导航和状态恢�?
 * @note timestamp 用于 LRU（最近最少使用）分析和调试追�?
 */
struct HistoryEntry {
    QString pageId;          ///< 页面唯一标识�?
 ...
 * @brief 页面导航控制器（单例模式�?
 * @details 核心职责�?
 * 1. 页面生命周期管理：创�?�?初始�?�?激�?�?失活 �?销�?缓存
 * 2. 智能缓存系统：双层缓存（强引�?弱引用）实现内存优化与快速响应的平衡
 * 3. 导航历史栈：支持复杂的页面回退场景（如表单填写中断后返回）
 * 4. 完全解耦：页面通过信号请求导航，无需知晓导航器存在（MVVM架构�?
 *
 * @warning 必须在主线程使用（QWidget限制），非线程安全设�?
 */
...
public:
     ...
     * @param container QStackedWidget 容器，负责实际页面堆叠显�?
     * @warning 必须在任何导航操作前调用，且只能初始化一�?
     * @note 初始化后会设置容器属�?WA_DeleteOnClose = false，防止容器被误删
     */
     ...
     * @note 对于 StrongCache 策略，会立即触发 preloadPage() 后台预加�?
     */
     ...
     * 3. 当前页面失活（deactivateCurrentPage：触�?onPageDeactivated，断开信号�?
     * 4. 新页面激活（activatePage：触�?onPageActivated，连接信号）
     ...
     * @note 会恢复上一次跳转时保存的参数（params），支持状态恢�?
     */
    bool navigateBack();

    /**
     * @brief 获取当前页面智能指针
     * @return 当前活动页面�?shared_ptr，若无则返回 nullptr
    ...
    /**
     * @brief 清空指定或全部页面缓�?
     * @param pageId 页面ID，为空字符串时清空所有缓�?
     * @details 强制立即释放内存，适用于：
     * - 内存警告响应（收到系统低内存通知时）
     * - 配置变更后需要重建页面（如语言切换、主题切换）
     * @warning 若清空当前活动页面的缓存，当前页面将保持显示直至下次导航，但下次访问会重新创�?
     */
...
signals:
     ...
     * @param cached true 表示被加入缓存，false 表示被移出缓�?
...
private slots:
     ...
     * @details 【关键设计】此槽函数是 BasePage::requestNavigation 信号的接收�?
     * 使用 Qt::UniqueConnection 确保同一页面多次激活不会建立重复连�?
     * @note 必须是槽函数（非Lambda）才能使�?UniqueConnection，这�?Qt 元对象系统限�?
     */
...
private:
     ...
     * 2. 弱缓存（m_weakCache）：尝试提升（lock()）为强引用，成功则复�?
     * 3. 工厂创建：调�?PageFactoryRegistry 创建新实�?�?执行 initializePage() �?按策略缓�?
     *
     * 性能优化�?
     ...
     * @param params 传递给页面的参�?
     * @details 执行操作�?
     * - 设置 QStackedWidget 当前索引（显示页面）
     * - 触发 onPageActivated(params)（页面可在此加载动态数据）
     * - 连接 requestNavigation 信号�?onPageRequestNavigation（Qt::UniqueConnection 防重连）
     */
     ...
     * - 若策略为 NoCache，立即从容器移除并释放资�?
     * @note 对于缓存策略页面，仅断开信号，保持实例存活以加速下次访�?
     */
    void deactivateCurrentPage();

    /**
     * @brief 清理过期弱缓�?
     * @details 定时器回调，扫描 m_weakCache 中已失效（expired）的 weak_ptr 并删�?
     * 防止长期运行下无效指针堆积造成的内存碎�?
     */
    ...
    // 双层缓存系统说明�?
    // m_strongCache: QMap<QString, shared_ptr> �?手动管理生命周期，常驻内存直�?clearCache 或应用退�?
    // m_weakCache: QMap<QString, weak_ptr> �?允许 Qt 对象树自动回收，但保留复用可�?
    ...
    QTimer *m_cleanupTimer = nullptr;  ///< 弱缓存清理定时器�?0秒间隔）
};
