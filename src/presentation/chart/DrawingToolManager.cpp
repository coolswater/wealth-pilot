...
void ChartDrawingToolManager::startDrawing(const QPointF& point)
{
...
void ChartDrawingToolManager::updateDrawing(const QPointF& point)
{
...
ChartDrawingObject ChartDrawingToolManager::finishDrawing(const QPointF& point)
{
    if (!m_isDrawing) {
...
void ChartDrawingToolManager::cancelDrawing()
{
    m_isDrawing = false;
    m_currentPoints.clear();
...
void ChartDrawingToolManager::addDrawing(const ChartDrawingObject& drawing)
{
    m_drawings.append(drawing);
    LOG_INFO(QString("Drawing added: %1").arg(drawing.id));
}

void ChartDrawingToolManager::updateDrawing(const QString& id, const ChartDrawingObject& drawing)
{
...
void ChartDrawingToolManager::deleteDrawing(const QString& id)
{
    for (int i = 0; i < m_drawings.size(); ++i) {
        if (m_drawings[i].id == id) {
...
void ChartDrawingToolManager::clearAllDrawings()
{
    m_drawings.clear();
    LOG_INFO("All drawings cleared");
}

ChartDrawingObject ChartDrawingToolManager::getDrawing(const QString& id) const
{
...
void ChartDrawingToolManager::selectDrawing(const QString& id)
{
    m_selectedId = id;
...
void ChartDrawingToolManager::deselectAll()
{
    m_selectedId.clear();
    emit selectionChanged(QString());
}

void ChartDrawingToolManager::setDefaultColor(const QColor& color)
{
    m_defaultColor = color;
}

void ChartDrawingToolManager::setDefaultLineWidth(int width)
{
    m_defaultLineWidth = width;
}

void ChartDrawingToolManager::setDefaultLineStyle(Qt::PenStyle style)
{
    m_defaultLineStyle = style;
}

QVector<FibonacciLevel> ChartDrawingToolManager::calculateFibonacciLevels(double high, double low) const
{
...
QString ChartDrawingToolManager::exportToJson() const
{
    QJsonArray array;
    for (const ChartDrawingObject& drawing : m_drawings) {
        QJsonObject obj;
...
bool ChartDrawingToolManager::importFromJson(const QString& json)
{
...
QString ChartDrawingToolManager::generateId() const
{
    return QUuid::createUuid().toString(QUuid::WithoutBraces);
}

ChartDrawingObject ChartDrawingToolManager::createDrawing(DrawingToolType type,
